...
class AABB {
  ...
  transform(mat) {
    ...
    vec3.set(Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, this.min);
    vec3.set(Number.MIN_VALUE, Number.MIN_VALUE, Number.MIN_VALUE, this.max);

    for (const corner of corners) {
      vec3.transformMat4(corner, mat, corner);
      vec3.min(this.min, corner, this.min);
      vec3.max(this.max, corner, this.min);
    }
  }
...
function setWorldMatrix(gltf, node, parentWorldMatrix) {
  ...
  if (node.matrix) {
    ...
    mat4.rotate(node.worldMatrix, node.rotation, node.worldMatrix);
    mat4.translate(node.worldMatrix, node.translation, node.worldMatrix);
    mat4.scale(node.worldMatrix, node.scale, node.worldMatrix)
  }
  ...
  mat4.transpose(mat4.invert(node.normalMatrix, node.normalMatrix), node.normalMatrix);
